{
  programs.git = {
    ...
    aliases = {
      ...
      bclean = "!f() { git for-each-ref --format='%(refname:short) %(upstream:track)' refs/heads | awk '$2 == \"[gone]\" {print $1}' | xargs -r git branch -D; }; f";
      bdone = "!f() { git checkout \${1-$(git default-branch-name)} && git up && git bclean; }; f";
